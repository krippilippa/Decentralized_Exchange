...
contract.skip ("Dex", accounts => {
    ...
    it("Should make sure the user has enough ETH when creating a buy order.", async () => {
        let dex = await Dex.deployed();
        let link = await Link.deployed();
        await truffleAssert.reverts(
    ...
    it("Should throw an error if user does not have enought tokens to create a sell order.", async () => {
        let dex = await Dex.deployed();
        let link = await Link.deployed();
        await truffleAssert.reverts(
            dex.createLimitOrder(1, web3.utils.fromUtf8("LINK"), 10, 200)
        )
        await link.approve(dex.address, 500)
    ...
    it("Should order the BUY orders correctly in the orderbook, highest to lowest.", async () => {
        let dex = await Dex.deployed();
        let link = await Link.deployed();
        ...
        assert(orderbook.length > 2, "no orders in orderbook");
        for(let i = 0; i < orderbook.length - 1; i++){
            assert(orderbook[i].price >= orderbook[i + 1].price, "not right price order");
        }
    })

    it("Should order the SELL orders correctly in the orderbook, lowest to highest.", async () => {
        let dex = await Dex.deployed();
        let link = await Link.deployed();
        await link.approve(dex.address, 1000)
        await dex.depositEth({value: 3000})
